...
const FriendRequests = ({
  ...
  const getRequests = (friendslist) => {
    return friendslist.filter(
      (friend) => friend.status === "Pending" && friend.user2 === user.userID
    );
  };
              ...
              {getRequests(user.friends).map((friend) => (
                <div>
                  <ListItem alignItems="flex-start" key={friend._id}>
                    <ListItemAvatar>
                      <Avatar>
                        {friend.user1name && friend.user1name.substring(0, 1)}
                      </Avatar>
                    </ListItemAvatar>
                    <ListItemText
                    ...
                    <ListItemSecondaryAction>
                      <IconButton
                        edge="end"
                        aria-label="delete"
                        onClick={() => acceptButton(friend.user1)}
                      >
                        <CheckIcon />
                      </IconButton>
                      <IconButton
                        edge="end"
                        aria-label="delete"
                        onClick={() => rejectButton(friend.user1)}
                      >
                        <ClearIcon />
                      </IconButton>
                    </ListItemSecondaryAction>
